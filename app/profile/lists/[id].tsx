...
const ListDetails = () => {
  ...
  const loadListAndMedia = async () => {
    ...
    try {
      ...
      Alert.alert("Error", "Failed to load list content.");
      console.error(error);
    } finally {
      setLoading(false);
    }
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              ...
              >
                  ...
                  >
                      <Image source={icons.trash} className="w-4 h-4" tintColor="#FF4444" />
                  </TouchableOpacity>
            ...
            ListEmptyComponent={() => (
              <View className="items-center mt-20">
                  <Image source={icons.clapperboard} className="w-16 h-16 opacity-20 mb-4" tintColor="white" />
                  <Text className="text-gray-100 font-pmedium">This list is empty.</Text>
                  <Text className="text-gray-100 text-xs mt-2">Add content by clicking the bookmark icon.</Text>
              </View>
            )}
          />
        )}
