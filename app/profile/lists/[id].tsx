...
const ListDetails = () => {
  const { id } = useLocalSearchParams(); // Pobieramy ID listy z URL
  const listId = Array.isArray(id) ? id[0] : id;

  const [listData, setListData] = useState<any>(null); // Dane listy (nazwa, opis)
  const [mediaItems, setMediaItems] = useState<any[]>([]); // Pobrane filmy/seriale
  ...
  const loadListAndMedia = async () => {
    setLoading(true);
    try {
      // 1. Pobierz dane listy z Appwrite
      const listDoc = await getListDetails(listId!);
      setListData(listDoc);

      // 2. Przetwórz tablicę items (np. ["movie:123", "tv:456"])
      const items = listDoc.items || [];
      
      // 3. Pobierz dane z TMDB dla każdego elementu równolegle
      const promises = items.map(async (itemStr: string) => {
        ...
        try {
          ...
          if (type === 'movie') {
            ...
            // Dodajemy pole 'mediaType' ręcznie, żeby wiedzieć co to jest
          ...
          return null; // Jeśli film usunięty z TMDB, zwróć null
        }
      });

      const results = await Promise.all(promises);
      // Odfiltruj ewentualne błędy (null)
        ...
        { text: "Anuluj", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              ...
              // Wywołanie API Appwrite
              await removeItemFromList(listId!, mediaId, type as 'movie' | 'tv');
              
              // Aktualizacja stanu lokalnego (bez przeładowania wszystkiego)
    ...
    <SafeAreaView className="bg-primary flex-1">
      ...
      {loading ? (
        ...
        <FlatList
          ...
          numColumns={2} // Wyświetlanie w dwóch kolumnach
